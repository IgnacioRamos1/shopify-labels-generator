...
def create_db_engine(stage):
    if 'AWS_EXECUTION_ENV' in os.environ:
        ...
        HOST = get_parameter(f'{stage}/db_host')
        PORT = get_parameter('db_port')
        DATABASE = get_parameter(f'{stage}/db_name')
        USER = get_parameter(f'{stage}/db_username')
        PASSWORD = get_parameter(f'{stage}/db_password')
    else:
        HOST = os.getenv(f'{stage}_DB_HOST')
        PORT = os.getenv(f'DB_PORT')
        DATABASE = os.getenv(f'{stage}_DB_NAME')
        USER = os.getenv(f'{stage}_DB_USERNAME')
        PASSWORD = os.getenv(f'{stage}_DB_PASSWORD')
